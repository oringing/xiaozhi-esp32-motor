...
// 方向控制引脚定义（8个，规避占用引脚）
#define MOTOR_DIR_PINS      {GPIO_NUM_9, GPIO_NUM_10, GPIO_NUM_11, GPIO_NUM_12, \
                             GPIO_NUM_13, GPIO_NUM_14, GPIO_NUM_8, GPIO_NUM_3}
...
public:
    ...
    // 参数：in1-in8为4个电机的方向控制引脚，ena/enb为PWM调速引脚
    MotorDriver(gpio_num_t in1, 
                gpio_num_t in2,  // 1号电机
                gpio_num_t in3, 
                gpio_num_t in4,  // 2号电机
                gpio_num_t in5, 
                ...
                gpio_num_t in8,  // 4号电机
                gpio_num_t ena, 
                gpio_num_t enb); // PWM调速引脚（ENA/ENB）
...
private:
    ...
    // 设置PWM占空比（内部调用）
    // 参数：ch - PWM通道，pin - 引脚，speed - 速度（0-255）
    void SetPwmSpeed(ledc_channel_t ch, 
                     gpio_num_t pin, 
                     uint8_t speed);
