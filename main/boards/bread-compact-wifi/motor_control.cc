...
// 声明嵌入的音频资源
extern const char spin_ogg_start[] asm("_binary_spin_ogg_start");
extern const char spin_ogg_end[]   asm("_binary_spin_ogg_end");
...
void MotorControl::InitializeTools() {
  ...
  // 顺时针转圈控制工具
  std::vector<Property> spin_clockwise_properties = {
    Property("speed", kPropertyTypeInteger, 255),  // 默认速度改为255
    Property("time", kPropertyTypeInteger, 2)
  };
  
  mcp_server.AddTool(
      "self.motor.spin_clockwise", "控制电机顺时针原地转圈。speed: 速度(0-255); time: 运行时间(0-100秒)",
      PropertyList(spin_clockwise_properties),
      [this](const PropertyList& properties) -> ReturnValue {
        int speed = properties["speed"].value<int>();
        int time = properties["time"].value<int>();
        SpinClockwise(speed, time);
        return "电机顺时针转圈，速度：" + std::to_string(speed) + "，时间：" +
               std::to_string(time) + "秒";
      });

  // 逆时针转圈控制工具
  std::vector<Property> spin_counterclockwise_properties = {
    Property("speed", kPropertyTypeInteger, 255),  // 默认速度改为255
    Property("time", kPropertyTypeInteger, 2)
  };
  
  mcp_server.AddTool(
      "self.motor.spin_counterclockwise", "控制电机逆时针原地转圈。speed: 速度(0-255); time: 运行时间(0-100秒)",
      PropertyList(spin_counterclockwise_properties),
      [this](const PropertyList& properties) -> ReturnValue {
        int speed = properties["speed"].value<int>();
        int time = properties["time"].value<int>();
        SpinCounterClockwise(speed, time);
        return "电机逆时针转圈，速度：" + std::to_string(speed) + "，时间：" +
               std::to_string(time) + "秒";
      });
  ...
  // 添加后退指定距离的工具
    std::vector<Property> move_backward_distance_properties = {
      Property("distance", kPropertyTypeString, "1.0"),
      Property("speed", kPropertyTypeInteger, 100)
    };
    
    mcp_server.AddTool(
      "self.motor.move_backward_distance", 
      "控制电机后退指定距离。distance: 距离(米, 0-5); speed: 速度(0-255)",
      PropertyList(move_backward_distance_properties),
      [this](const PropertyList& properties) -> ReturnValue {
        std::string distance_str = properties["distance"].value<std::string>();
        float distance = std::stof(distance_str);
        int speed = properties["speed"].value<int>();
        MoveDistance(-distance, speed);  // 使用负数距离表示后退
        std::string result = "电机后退指定距离：" + std::to_string(distance) + "米，速度：" +
               std::to_string(speed);
        return result;
      });
...
// 添加顺时针转圈方法实现
void MotorControl::SpinClockwise(int speed, int time) {
  SetParameters(speed, time);
  ...
  ESP_LOGI(TAG, "Spinning clockwise: speed=%d, run_time=%d", motor_speed_, run_time_);
}
...
void MotorControl::SpinCounterClockwise(int speed, int time) {
  ...
  ESP_LOGI(TAG, "Spinning counter-clockwise: speed=%d, run_time=%d", motor_speed_, run_time_);
}
  ...
  while (current_distance < target_meters) {
    ...
    if (current_distance >= (target_meters - braking_distance)) {
      ...
      if (target_meters >= 0) {
        ...
        motor_driver_.Backward(reduced_speed);
      }
    }
