...
MotorControl::MotorControl() 
    : motor_driver_(GPIO_NUM_9, 
                   ...
                   GPIO_NUM_3,   // 4号电机
                   GPIO_NUM_4, 
                   GPIO_NUM_5) { // ENA/ENB引脚
    // 初始化电机驱动
    motor_driver_.Init();

    // 创建停止定时器（单次触发）
    stop_timer_ = xTimerCreate("MotorStopTimer",
                               pdMS_TO_TICKS(1000),
                               pdFALSE,
                               this,
                               StopMotorTimerCallback);
    if (!stop_timer_) {
...
void MotorControl::StopMotorTimerCallback(TimerHandle_t timer) {
    auto* motor_control = static_cast<MotorControl*>(pvTimerGetTimerID(timer));
    if (motor_control) {
...
void MotorControl::InitializeTools() {
    ...
    mcp_server.AddTool("self.motor.forward",
                      "控制电机前进。speed: 速度(0-255); time: 运行时间(0-100秒)",
                      PropertyList({
                      ...
                      [this](const PropertyList& properties) -> ReturnValue {
                          ...
                          Forward(speed, time);
                          return "电机前进，速度：" + std::to_string(speed) + 
                                 "，时间：" + std::to_string(time) + "秒";
                      });

    // 后退控制工具
    mcp_server.AddTool("self.motor.backward",
                      "控制电机后退。speed: 速度(0-255); time: 运行时间(0-100秒)",
                      PropertyList({
                          Property("speed", kPropertyTypeInteger, 100, 0, 255),
                          Property("time", kPropertyTypeInteger, 2, 0, 100)
                      }),
                      [this](const PropertyList& properties) -> ReturnValue {
                          ...
                          int time = properties["time"].value<int>();
                          Backward(speed, time);
                          return "电机后退，速度：" + std::to_string(speed) + 
                                 "，时间：" + std::to_string(time) + "秒";
                      });

    // 左转控制工具
    mcp_server.AddTool("self.motor.turn_left",
                      "控制电机左转。speed: 速度(0-255); time: 运行时间(0-100秒)",
                      PropertyList({
                          Property("speed", kPropertyTypeInteger, 100, 0, 255),
                          Property("time", kPropertyTypeInteger, 2, 0, 100)
                      }),
                      [this](const PropertyList& properties) -> ReturnValue {
                          ...
                          int time = properties["time"].value<int>();
                          TurnLeft(speed, time);
                          return "电机左转，速度：" + std::to_string(speed) + 
                                 "，时间：" + std::to_string(time) + "秒";
                      });

    // 右转控制工具
    mcp_server.AddTool("self.motor.turn_right",
                      "控制电机右转。speed: 速度(0-255); time: 运行时间(0-100秒)",
                      PropertyList({
                          Property("speed", kPropertyTypeInteger, 100, 0, 255),
                          Property("time", kPropertyTypeInteger, 2, 0, 100)
                      }),
                      [this](const PropertyList& properties) -> ReturnValue {
                          ...
                          return "电机右转，速度：" + std::to_string(speed) + 
                                 "，时间：" + std::to_string(time) + "秒";
                      });
